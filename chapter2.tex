...
(or 1). I prefer the string versions are they are more explicit and tend to make the code easier
to read.

Nearly all DataFrame methods default the axis parameter to 0, which applies to operations
along the index.
\subsection{How to do it\dots}
\begin{enumerate}
\item
The .count method returns the
...
The axis parameter is almost always set to 0. So, step 2 is equivalent to both
\texttt{college\_ugds.count(axis=0)}
and \texttt{college\_ugds.count(axis='indexs')}
...
can use the .dropna method to drop all rows that have all 
...
\item
To get an idea of the distribution, we will use the .value\_counts method on this
Series.

\item
Amazingly, two schools have more than 15\% in five different race categories. Let's
sort the diversity\_metric Series to find out which ones they are:
\begin{py}{}
college_ugds.ge(.15)

diversity_metric = college_ugds.ge(.15).sum(
    axis='columns'
)
diversity_metric.head(7)

diversity_metric.value_counts()

diversity_metric.sort_values(ascending=False).head(5)
\end{py}
\item
